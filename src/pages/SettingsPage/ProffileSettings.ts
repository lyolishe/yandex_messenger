...
const createProfileSettings = (): Block => {
    const appWrapper = new Wrapper({ classList: ['appMainWrap'] });
    const appManin = new Wrapper({ classList: ['appMain'] });

    const user = Context.instance.get('user') as UserResponse;

    const changePasswordForm = new Form({ id: 'ChangePassword' }, changePasswordTmpl);
    const onChangePassword = () => {
        useApi<void>(UsersApi.changePassword(changePasswordForm.getFieldsValue()));
    };

    const profileForm = new Form({ id: 'ProfileSettings', initialValues: user }, profileSettingsTmpl);
    const onSubmitProfile = () => {
        useApi<UserRequest>(UsersApi.changeProfile(profileForm.getFieldsValue())).then((res) => Context.instance.set('user', res));
    };
    ...
    const nav = new Block('nav', {
        classList: ['navbar'],
        children: [new UserBlock({ first_name: 'Profile' })],
    });
...
export default createProfileSettings;

...
export class Validation {
    ...
    private _checkValidity = ():void => {
        this._errors = [];
        const { validity } = this._input;
        for (const state in validity) {
            if (validity[state as keyof ValidityState] && state !== 'valid') {
                this._addError(this.validationMessages?.[state as keyof ValidityState]
                    ?? splitCamelCase(state).toLocaleLowerCase());
            }
        }
